...
export default function CallbackForm({ setActive }) {
  ...
  return (
    <section className="callback">
      ...
      <p className="article">Отправьте заявку на обратный звонок, наши специалисты свяжутся с вами в ближайшее
        время.</p>
      <form ref={form} className="callback__form" onSubmit={(e) => {
        ...
        <div className="callback__buttons">
          ...
          <button type="button" className="callback__button callback__button--close"
                  onClick={() => setActive(false)}>Закрыть
          </button>
        </div>
        <p className={sent ? "callback__notify article active" : "callback__notify article"}>Ваша заявка отправлена!</p>
      </form>
    </section>
  );
}